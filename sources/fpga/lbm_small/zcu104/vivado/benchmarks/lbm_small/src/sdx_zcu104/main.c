...
int main( int nArgs, char* arg[] ) {
	...
	if(retVal) goto _err;
	retVal = clCopyToDevice((float *) srcGrid, (float *) dstGrid);
	if(retVal) goto _err;
	...
	retVal = clCopyFromDevice((float *) srcGrid, (float *) dstGrid);
...
void MAIN_parseCommandLine( int nArgs, char* arg[], MAIN_Param* param, struct pb_Parameters * params) {
	...
	if( nArgs < 2 ) {
		printf( "syntax: lbm_small <time steps>\n" );
		exit( 1 );
	}
