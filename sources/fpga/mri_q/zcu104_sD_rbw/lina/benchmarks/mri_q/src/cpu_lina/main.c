...
main (int argc, char *argv[]) {
	...
	char *_argv[] = {
		argv[0],
		"-i",
		"32_32_32_dataset.bin",
		"-o",
		"32_32_32_dataset.out",
		"--",
		NUMK_STR
	};
  ...
  if(numX != NUMX || numK != NUMK) {
    printf("ERROR: This version has fixed dimensions. numX must be %d, numK must be %d\n", NUMX, NUMK);
    exit(-1);
  }
  ...
  mri_q(kx, ky, kz, phiMag, x, y, z, Qr, Qi);
