...
<HEADER>
	...
	for(unsigned iter = 0; iter < NUM_ITERATIONS; iter++) {
		...
		<LOOP_0_1> for(unsigned int i = 0; i < (IMG_WIDTH * IMG_HEIGHT); i += <LINCR>) {
			<LPREAMB>
			<LBEGIN>
			const unsigned int __tmp_<LCTR> = <ARR_img>[i + <LITER>];
			<LFRONT>
			if(<ARR_hist>[__tmp_<LCTR>] < 255)
				++<ARR_hist>[__tmp_<LCTR>];
			<LEND>
		}
	}
